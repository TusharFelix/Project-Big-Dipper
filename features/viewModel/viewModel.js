...
function viewMobileDetails(btnEle) {
  ...
  var brand = btnEle.parentElement.parentElement.getElementsByClassName(
    "brand-title"
  )[0].innerText;
...
function mobileObj(brandObj, brand, mobileName, mobile_ram, mobile_price) {
  brandObj.forEach(function(element) {
    ...
    if (
      element.model == mobileName &&
      element.ram == mobile_ram &&
      element.price == mobile_price
    ) {
...
function modelData(mobileObject, brand) {
  ...
  mobile_desc.querySelector("#mobile-processor").innerText =
    mobileObject.processor;
  mobile_desc.querySelector("#mobile-screensize").innerText =
    mobileObject.screenSize;
  mobile_desc.querySelector("#mobile-simType").innerText = mobileObject.simType;
  mobile_desc.querySelector("#mobile-otgcompatible").innerText =
    "OTG : " + mobileObject.otg_Compatible;

  //specification
  mobile_spec.querySelector("#mobile-price").innerText =
    "PRICE : Rs." + mobileObject.price;
  mobile_spec.querySelector("#mobile-Rating").innerText =
    "RATINGS : " + mobileObject.ratings;
  mobile_spec.querySelector("#mobile-ModelNo").innerText =
    "MODELNUMBER : " + mobileObject.modelNumber;
  mobile_spec.querySelector("#mobile-color").innerText =
    "COLOR : " + mobileObject.color;
}
